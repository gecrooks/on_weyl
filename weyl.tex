...
\node [below, rotate=-90, magenta] at (1, -{3/5}) {Improper orthogonal gates $(\tfrac{1}{2}, t_y, t_z)$};
\node [above, rotate=15] at ({1+sqrt(2)/4}, -{sqrt(2)/4}) {Special orthogonal gates $(t_x, t_y, 0)$};

% CPHASE
\node [above, Maroon] at ({1/2}, 0) {$\Gate{CPHASE}\quad(t,0,0)$};
...
\node at (1, 0.48){\small{\url{http://threeplusone.com/weyl}}};
\node at (1, 0.40){\small{Gavin E. Crooks (2019)}};
\node at (1, 0.32){\small{Tech. Note 12v2}};
